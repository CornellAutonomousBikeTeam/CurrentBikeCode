...
class IMU
{
  private:
    ...
    const int delta = 180/3.14; //Need to add division fn
    const byte INST = 0x01;  //Read filtered, tared orientation (Euler Angles)// 0x01, 0x02, 0x04, 0x08 (3, 6, 9, 10)
    const byte instructions[4] = {0x01, 0x02, 0x04, 0x08};
    const float flinst[4] = {3, 9, 6, 9};
  public:  
    ...
    union u_types {
      ...
      float fval;
        ...
        //can't have a set up, can't have a loop
    void IMUClasssetup() {
      //Set Pin Modes
      pinMode(CSN, OUTPUT);
      pinMode(SI, OUTPUT);
