...
int total_millis = 0; //used to determine if we are running at 5hz
int count = 0;
int prev_millis = 0;
int curr_millis = 0;
int bits_so_far = 0;
//ros::NodeHandle  nh;
ros::NodeHandle_<ArduinoHardware, 1, 3, 5000, 5000> nh; //set this back to 500 
...
void loop() {
  ...
  // the nav algorithm
  bike_state.data[0] = 0; //front motor (rad/s)
  bike_state.data[1] = 0; //front motor (rad/s)
  bike_state.data[2] = 0; //front motor (rad) (delta)
  bike_state.data[3] = 0; //front motor (rad)
  bike_state.data[4] = imu_data.rate; //imu (rad) (phi-dot)
  bike_state.data[5] = imu_data.angle; //imu (rad/s) (phi)
  bike_state.data[6] = .1; //rear motor (m/s) (based on hall sensor)
  bike_state.data[7] = 0; //rear motor commanded speed (pwm)
 ...
 // }
  ...
  //Serial.print("GPS DATA: "); Serial.println(sizeof(gps_state ));
  //Serial.print("BIKE DATA: "); Serial.println(sizeof(bike_state ));
  //Serial.print("PID CONTROLLER DATA: "); Serial.println(sizeof(pid_controller_data));
  //Serial.print("TOTAL DATA: ");  Serial.println(((sizeof(gps_state) + sizeof(bike_state) + sizeof(pid_controller_data))) * 8);
  ...
  //Serial.println(curr_millis - prev_millis);
